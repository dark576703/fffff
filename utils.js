    ...
    return function(/** @type {{ body: string; statusCode: string | number; request: { uri: { protocol: string; hostname: string; pathname: string; }; headers: { [x: string]: string; }; formData: any; method: string; }; }} */data) {
        return bluebird.try(function() {
            ...
            if (res.error === 1357001) {
                switch (globalThis.Fca.Require.Cyber.AutoLogin) {
                    case true: {
...
function getAppState(jar, Encode) {
    ...
    var Language = languageFile.find(i => i.Language == globalThis.Fca.Require.Cyber.Language).Folder.Index;
    var data;
        switch (require(process.cwd() + "/CyberFca.json").EncryptFeature) {
            case true: {
            ...
            case false: {
                ...
                break;
            default: {
                logger.Normal(getText(Language.IsNotABoolean,require(process.cwd() + "/CyberFca.json").EncryptFeature));
                data = appstate;
            } 
        }
            if(!globalThis.Fca.Setting.get('getAppState')) {
