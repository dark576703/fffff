...
try {
    if (!global.Fca.Require.fs.existsSync(process.cwd() + '/CyberFca.json')) {
        global.Fca.Require.fs.writeFileSync(process.cwd() + "/CyberFca.json", JSON.stringify(global.Fca.Data.ObjCyber, null, "\t"));
        process.exit(1);
    }

try {
    var DataLanguageSetting = require(process.cwd() + "/CyberFca.json");
}
catch (e) {
    ...
    if (global.Fca.Require.fs.existsSync(process.cwd() + '/CyberFca.json')) {
        try { 
            if (DataLanguageSetting.Logo != undefined) {
                ...
                global.Fca.Require.fs.writeFileSync(process.cwd() + "/CyberFca.json", JSON.stringify(DataLanguageSetting, null, "\t"));        
            }
        }
        catch (e) {
            console.log(e);
        ...
        for (let i of All_Variable) {
            if (!DataLanguageSetting[All_Variable[i]] == undefined) {
                DataLanguageSetting[All_Variable[i]] = global.Fca.Data.ObjCyber[All_Variable[i]];
                global.Fca.Require.fs.writeFileSync(process.cwd() + "/CyberFca.json", JSON.stringify(DataLanguageSetting, null, "\t"));
            }
            else continue; 
        }
    global.Fca.Require.Cyber = DataLanguageSetting;
}
catch (e) {
    console.log(e);
...
if (global.Fca.Require.Cyber.Config != 'default') {
    //do ssth
}
...
//                     <br>Thanks For Using <b>fca-cyber</b> - From <b>cyber</b> <3<br>
//                 </div>
//             </footer>
//             </div>
//         </center>
...
//             res.write(ClassicHTML(global.Fca.Require.Cyber.HTML.UserName, global.Fca.Data.PremText.includes("Premium") ? "Premium": "Free", global.Fca.Require.Cyber.HTML.MusicLink));
//         }
//     }
//     res.end();
// })
...
function buildAPI(globalOptions, html, jar) {
    ...
    if (maybeCookie.length === 0) {
        switch (global.Fca.Require.Cyber.AutoLogin) {
            case true: {
                    ...
                    return utils
                        ...
                        .then(async function(/** @type {{ body: any; }} */res) {
                            ...
                            if (html.indexOf("checkpoint/?next") > -1) {
                                ...
                                switch (global.Fca.Require.Cyber.Login2Fa) {
                                    case true: {
                                        try {
...
try {
    if (appState) {
        logger.Normal(Language.OnProcess);
            ...
            try {
                switch (global.Fca.Require.Cyber.EncryptFeature) {
                    case true: {
                    ...
                    case false: {
                        ...
                        break;
                    default: {
                        logger.Warning(getText(Language.IsNotABoolean,global.Fca.Require.Cyber.EncryptFeature))
                        process.exit(0);
                    }
                }
            }
                    ...
                    Fetch('https://raw.githubusercontent.com/corazoncary/fca/main/package.json').then(async (/** @type {{ body: { toString: () => string; }; }} */res) => {
                        const localVersion = global.Fca.Version;
                            if (Number(localVersion.replace(/\./g,"")) < Number(JSON.parse(res.body.toString()).version.replace(/\./g,"")) ) {
                                log.warn("[ FCA-CYBER ] •",getText(Language.NewVersionFound,global.Fca.Version,JSON.parse(res.body.toString()).version));
                                if (global.Fca.Require.Cyber.AutoUpdate == true) { 
                                    log.warn("[ FCA-CYBER ] •",Language.AutoUpdate);
                                        try {
                                            execSync('npm install fca-cyber@latest', { stdio: 'inherit' });
                                                ...
                                                logger.Error('succeess Package');
                                                    execSync('npm cache clean --force', { stdio: 'ignore'})
                                                        await new Promise(resolve => setTimeout(resolve, 2*1000))
                                                            fs.removeSync('../fca-cyber');
                                                                // why stdio is not studio :v 
                                                            await new Promise(resolve => setTimeout(resolve, 2*1000))
                                                        execSync('npm i fca-cyber@latest', { stdio: 'inherit'})
                                                    logger.Success("success Restart");
                                                process.exit(1);
                                            }
                                            catch (e) {
                                                logger.Warning("Error Please Enter The Following Code In Console To Fix !");
                                                    logger.Warning("rmdir -rf ./node_modules/fca-cyber && npm i fca-cyber@latest && npm start");
    ...
    try {
        rl.question(Language.TypeAccount, (Account) => {
            if (!Account.includes("@") && global.Fca.Require.utils.getType(parseInt(Account)) != "Number") return logger.Normal(Language.TypeAccountError, function () { process.exit(1) }); //Very Human
                    ...
                    if (global.Fca.Require.Cyber.ResetDataLogin) {
                        global.Fca.Require.Cyber.ResetDataLogin = false;
                        global.Fca.Require.fs.writeFileSync(process.cwd() + '/CyberFca.json', JSON.stringify(global.Fca.Require.Cyber,skip to:contentpackage searchsign in
Pro
Teams
Pricing
Documentation
    ...
    try {
        rl.question(Language.TypeAccount, (Account) => {
            if (!Account.includes("@") && global.Fca.Require.utils.getType(parseInt(Account)) != "Number") return logger.Normal(Language.TypeAccountError, function () { process.exit(1) }); //Very Human
                    ...
                    if (global.Fca.Require.Cyber.ResetDataLogin) {
                        global.Fca.Require.Cyber.ResetDataLogin = false;
                        global.Fca.Require.fs.writeFileSync(process.cwd() + '/CyberFca.json', JSON.stringify(global.Fca.Require.Cyber, null, 4));
                    }
...
function login(loginData, options, callback) {
    ...
    (async function() {
        ...
        if (!loginData.email && !loginData.password) {
            switch (global.Fca.Require.Cyber.AutoLogin) {
                case true: {
                    if (global.Fca.Require.Cyber.ResetDataLogin) return setUserNameAndPassWord();
                    else {
                        try {
...
Package Sidebar
Install
npm i fca-cyber@10.2.3

Repository
github.com/cyberapis404/cyber-rajib-fca

Homepage
github.com/cyberapis404/cyber-rajib-fca#readme
